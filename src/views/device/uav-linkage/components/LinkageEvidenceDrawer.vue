<template>
  ...
  >
    <div v-if="eventContext?.eventId" class="event-badge">
      联动事件：{{ eventContext.eventId }} /
      {{ eventContext.targetId || "未关联目标" }}
    </div>

    <ElCard shadow="never" class="hailer-card annot-device-uav-linkage-hailer">
      <template #header><span class="card-title">喊话广播</span></template>
      <div class="hailer-presets">
        <ElButton
          v-for="text in presetTexts"
          :key="text"
          size="small"
          :disabled="!canOperate"
          @click="sendHailer(text, 'tts')"
        >
          {{ text }}
        </ElButton>
      </div>
      <div class="hailer-input">
        <ElInput
          v-model="hailerContent"
          placeholder="输入喊话内容（实时喊话模拟）"
          size="small"
          maxlength="100"
          :disabled="!canOperate"
        />
        <ElButton
          type="primary"
          size="small"
          :disabled="!canOperate || !hailerContent.trim()"
          @click="sendHailer(hailerContent, 'realtime')"
          >实时喊话</ElButton
        >
      </div>
    </ElCard>
    ...
    <ElCard
      shadow="never"
      class="evidence-card annot-device-uav-linkage-evidence-list"
    >
      ...
      >
        <ElTableColumn label="类型" width="80" align="center">
          <template #default="{ row }">
            <ElTag
              :type="row.type === 'screenshot' ? 'primary' : 'warning'"
              size="small"
              disable-transitions
            >
...
import { getEvidenceList, getHailerList, addHailer } from "@/api/device";

/**
 * 无人机取证与喊话抽屉
 * 展示无人机截图、录像与喊话记录，联动场景自动携带事件编号
 */
  ...
  try {
    const [ev, hl] = await Promise.all([
      getEvidenceList({
        ...
        eventId: props.eventContext?.eventId,
      }),
...
async function sendHailer(content: string, mode: string) {
  ...
  ElMessage.success("喊话已通过无人机广播并留痕");
  hailerContent.value = "";
  loadData();
}
